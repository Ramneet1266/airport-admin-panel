...
export default function Page() {
	...
	useEffect(() => {
		...
		// Directly call async function inside useEffect
		;(async () => {
		  ...
		  try {
			...
			console.log("importUsersFromFirestore completed")
		  } catch (error) {
			console.error("Error in importUsersFromFirestore:", error)
		  }
		})()
	  }, [])
    ...
    try {
        ...
        if (res) {
            ...
            await sendStoreCredentials(storeHeadEmail, email, password);

            toast.success("Store added & credentials sent!");
            setIsOpen(false);
            clearFormFields();
            await fetchAgents();
        }
